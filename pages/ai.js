  ...
  return (
    <div style={{minHeight:'100vh',background:'#111',fontFamily:'Poppins,sans-serif'}}>
      <div style={{background:'#0a0a0a',padding:'1rem 2rem',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'3px solid #D06830'}}>
        <div style={{fontFamily:'Poppins,sans-serif',fontSize:16,color:'#fff',fontWeight:700,letterSpacing:'.08em'}}>SPANGLERBUILT <span style={{fontSize:11,color:'#D06830',fontWeight:400}}>AI TOOLS</span></div>
      ...
      <div style={{padding:'1.5rem',maxWidth:1000,margin:'0 auto'}}>
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:'1.25rem'}}>
          {ACTIONS.map(function(a){return (
            <button key={a.id} onClick={function(){setAction(a.id)}} style={{padding:'6px 14px',fontSize:11,border:'1px solid',fontFamily:'inherit',fontWeight:500,cursor:'pointer',borderRadius:3,borderColor:action===a.id?'#0a0a0a':'#e8e6e0',background:action===a.id?'#0a0a0a':'#fff',color:action===a.id?'#D06830':'#5f5e5a'}}>{a.label}</button>
          )})}
        </div>
        <div style={{background:'#161616',border:'1px solid rgba(255,255,255,.09)',borderRadius:4,padding:'1.5rem',marginBottom:'1rem'}}>
          <div style={{fontSize:10,fontWeight:500,color:'rgba(255,255,255,.35)',textTransform:'uppercase',letterSpacing:'.1em',marginBottom:8}}>Describe your project</div>
          <textarea value={prompt} onChange={function(e){setPrompt(e.target.value)}} onKeyDown={onKey}
            ...
            style={{width:'100%',fontFamily:'inherit',fontSize:13,border:'1px solid rgba(255,255,255,.09)',borderRadius:3,padding:'10px',minHeight:100,outline:'none',resize:'vertical',marginBottom:12,background:'rgba(208,104,48,.1)'}}/>
