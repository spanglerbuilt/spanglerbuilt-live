export default function ScopePage() {
  ...
  const STATUS = {
    ...
    'in progress':{bg:'#fff3e0',color:'#FF8C00',dot:'#FF8C00',label:'In progress'},
  ...
  return (
    <div style={{minHeight:'100vh',background:'#fff',fontFamily:'sans-serif'}}>
      ...
      <div style={{maxWidth:800,margin:'0 auto',padding:'1.5rem'}}>
        <div style={{background:'#002147',borderRadius:4,padding:'1rem 1.5rem',marginBottom:'1.25rem',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
          <div>
            <div style={{fontFamily:'Georgia,serif',fontSize:16,color:'#fff',marginBottom:2}}>Mendel Basement Renovation</div>
            <div style={{fontSize:11,color:'rgba(255,255,255,.6)'}}>SB-2026-001 · 665 sf · 5 phases · Feb 9 – Apr 22, 2026</div>
          </div>
          <div style={{display:'flex',gap:12}}>
            {[['Complete','#3B6D11'],['In progress','#FF8C00'],['Upcoming','#9a9690']].map(([l,c])=>(
        ...
        {phases.map(phase=>{
          ...
          return (
            <div key={phase.num} style={{background:'#fff',border:'1px solid #e8e6e0',borderRadius:4,overflow:'hidden',marginBottom:10}}>
              <div style={{background:sc.bg,padding:'10px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #e8e6e0'}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:24,height:24,background:'#002147',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#FF8C00',flexShrink:0}}>{phase.num}</div>
                  <span style={{fontSize:13,fontWeight:500,color:'#002147'}}>{phase.name}</span>
                </div>
              ...
              <div style={{padding:'10px 14px'}}>
                {phase.items.map((item,i)=>(
                  <div key={i} style={{display:'flex',gap:8,alignItems:'flex-start',padding:'5px 0',borderBottom:i<phase.items.length-1?'1px solid #f5f4f1':'none',fontSize:12,color:phase.status==='upcoming'?'#9a9690':'#3d3b37'}}>
                    <span style={{color:phase.status==='complete'?'#3B6D11':phase.status==='in progress'?'#FF8C00':'#e8e6e0',fontSize:12,flexShrink:0,marginTop:1}}>
